...
const cartSlice = createSlice({
    ...
    reducers:{
        addtoCart:(state,obj)=>{
            ...
            const requiredProduct = state.cartProducts.find((product)=>{
                product.id=productToBeAdded.id
            })
            if(requiredProduct){
                productToBeAdded.qty++;
            }
            else{
        ...
        removefromCart:()=>{
            ...
            const productIdx = state.cartProducts.findIndex(product=>product.id==productToBeRemoved)
            if(productIdx!=-1){
                state.cartQuantity--;
